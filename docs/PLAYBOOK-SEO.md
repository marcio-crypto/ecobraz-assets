...
| 2026-07-19 | Ecobraz | **Kits de backlink prontos:** `docs/PR/kit-parceiro-ecobraz.md` (selo HTML autossuficiente — verificado o render + textos p/ clientes linkarem) e `docs/PR/kit-cadastro-diretorios-ecobraz.md` (NAP padrão + 3 descrições + categorias, copiar/colar). Nome registrado confirmado: **Ecobraz Emigre**. | Depende do Marcio: enviar aos clientes / cadastrar nos diretórios (login é dele) |
| 2026-07-20 | Ecobraz | **Recuperação de link (404 → 301):** auditoria de backlinks quebrados (Ahrefs) achou 2 links editoriais de alto valor apontando para `/conheca-a-ong/quem-somos` (site antigo, dava 404): **iFixit DR83** e **TechTudo DR80**. Criado **redirect 301 no Cloudflare** (Ghost está no limite de 9.069 regras) → `/sobre/`. **Testado e funcionando** (Marcio confirmou). O resto dos backlinks quebrados = rede de spam "Reciclagem de Cabos e Fios" → deixados em 404 de propósito (não recuperar spam). **Varredura profunda (ordenada por tráfego) confirmou: os 2 links reais recuperados são os ÚNICOS de valor**; demais alvos 404 são spam ou domínios DR0-26 negligíveis (ex.: /projetos/.../coleta-gratuita, /component/k2/... de spam polonês). Nenhum backlink aponta para /public/ (a regra Cloudflare desabilitada não recupera link nenhum). | Aguardar Google re-rastrear p/ repassar a autoridade (semanas) |
| 2026-08-19 | Villanova | **Registros 21 e 22 da série ESCRITOS (não depositados):** #3 *From MTR/SINIR to the European Buyer* — crosswalk dos instrumentos brasileiros (MTR, CDF, LO/CADRI, NBR 10004/16156) contra as classes de pedido europeu, com as 6 lacunas e o conteúdo do arquivo legível pelo comprador; inclui a afirmação de que desmatamento (2023/1115) e CBAM (2023/956) **provavelmente não se aplicam** a serviço de destinação. #4 *Documenting Avoided Emissions* — protocolo de 9 elementos (linha de base explícita, fatores rastreáveis, registro de premissas) + tabela do que se pode e não se pode alegar; **sem nenhum número**, porque o módulo de cálculo da Ecobraz é fase de plano. Marcio informou OK dos especialistas sobre a **base de pesquisa**; o crosswalk em si ainda não foi lido por eles. | Marcio: leitura final dos especialistas sobre o #3; depositar na ordem 1→2→3→4 |
| 2026-08-19 | Villanova | **Série de DOI planejada + registros 19 e 20 ESCRITOS (não depositados):** plano em `docs/villanova/PLANO-SERIE-DOI.md` — 4 registros no núcleo + 2 opcionais, regra "publicar o método, não o produto" (o Sistema da Ecobraz entra só no registro #5, como implementação de referência declarada, com conflito de interesse). **Escritos e com PDF gerado** (EN+PT, `docs/villanova/serie-doi/`): #1 *Evidence Architecture* (tabela de eventos de captura + 3 regras de ligação) e #2 *Minimum Specification for Verifiable Certificates* (10 requisitos + lista de conformidade). Nenhum depósito feito — é sob o ORCID do Marcio. | Marcio: conferir os ⚠️, depositar o #1, mandar o DOI (o #2 cita o #1 e só depois vai) |
| 2026-07-22 | Ecobraz | **Incidente "broken JavaScript" (auditoria 22/jul):** ~3.348 páginas marcadas "has broken JavaScript" = **1 arquivo só** referenciado em todo o site: `/public/cards.min.js` (**404**, não existe). **NÃO está no nosso tema/repo** (verificado) → foi injetado **via Code Injection do Ghost** (admin), coerente com "GPT/ads" que o Marcio colou. Migração intacta (verify-migration verde; páginas 200). Correção = remover o `<script ...cards.min.js...>` do Code Injection (depende do Marcio; não mexo no site sem aprovação). | Marcio: remover o script no Ghost (ou colar aqui p/ eu ver); dizer o que queria com "ads" p/ fazer certo |
| 2026-07-20 | Ecobraz | **Rotina semanal (migração informa→ecobraz):** `informa-virada.yml` = **303 redirects corretos, 2 erros** — ambos do colunista **Sergio Diniz** (`/colunista/7/sergio-diniz` [+/amp] → `/autor/sergio-diniz/` que dá 404). É o **item manual conhecido** (autor não migrado; só 4 autores em `autor-pages.json`). Migração 99,3% saudável. **Relatório de conteúdo NÃO gerado:** GSC↔Ahrefs sem conexão ("No GSC data available"). | Marcio decide: criar página do autor Sergio Diniz OU redirecionar p/ fallback; e conectar GSC↔Ahrefs (ou exportar do GSC) |
| 2026-07-20 | Ecobraz | **5XX e 404 diagnosticados (GSC filtrado):** **5XX RESOLVIDO** — zero erros de servidor pós-migração (14–17/jul = 0,0,0,0); os 5xx eram do **site antigo (jun/início jul)**, agora estável. **404** = ~12,4 mil no período, **~81% são fantasmas do site multilíngue antigo** (/es/ /en/ /de/ /fr/ /pt_BR/ /zh/) + junk de CMS antigo (/public/ /wp-content/ /storage/). Nenhum tem backlink de valor (auditado) → **404 é o correto; deixar como está** (não redirecionar em massa; regra /public/ fica desabilitada). Site novo saudável. | Nenhuma ação; Google vai reduzir o rastreio dos fantasmas com o tempo |
| 2026-07-20 | Villanova | **Diagnóstico de rastreamento (GSC Crawl Stats):** Villanova NÃO está travada. Googlebot rastreia ativamente, sem erro 5xx, com pico em 16–17/jul (399→3.397 req/dia) após o empurrão de conteúdo. Pouco rastreio vs. Ecobraz = domínio novo + autoridade real ~zero (esperado), não bloqueio. Os **12% de respostas 302 são 100% beacons do Google Analytics** (`/g/measurement/`, ID G-Y1ERW8GNPR) — não são páginas; comportamento normal. **Decisão: deixar como está** (inofensivo; robots.txt não vale o esforço). | Sem ação; medir crawl subir conforme autoridade cresce |
