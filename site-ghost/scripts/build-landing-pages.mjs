...
const depoimentosSection = (p, slug) => !p.depoimentos ? '' : `<section class="hx-block">
  <div class="container">
    <div class="hx-head-split">
      ...
      <p><span style="color:#F5B940">★</span> <strong>${esc(p.depoimentos.nota)}</strong> em <a class="hx-src" href="${p.depoimentos.url}" rel="noopener" data-track="${slug}_avaliacoes_google">${esc(p.depoimentos.total)} avaliações públicas no Google em ${esc(p.depoimentos.data || 'set/2026')} →</a></p>
    </div>
    <div class="hx-docs">
...
`;
const depoimentosHtml = (p) => !p.depoimentos ? '' : `<hr><h2>Avaliações públicas no Google</h2><p>Nota ${esc(p.depoimentos.nota)} em ${esc(p.depoimentos.total)} avaliações públicas em ${esc(p.depoimentos.data || 'set/2026')}.</p>${p.depoimentos.itens.map((r) => `<blockquote><p>“${esc(r.texto)}” — ${esc(r.autor)}</p></blockquote>`).join('')}<p><a href="${p.depoimentos.url}" rel="noopener">Ver todas as avaliações no Google →</a></p>`;
