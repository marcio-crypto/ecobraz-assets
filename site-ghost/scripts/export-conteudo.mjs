// Exporta todo o conteúdo do Ghost da Ecobraz (páginas e posts, com HTML)
// para JSON em site-ghost/export/ — insumo da versão EN do site.
...
await fs.mkdir('site-ghost/export', {recursive: true});
const pages = await listar('pages');
const posts = await listar('posts');
await fs.writeFile('site-ghost/export/pages.json', JSON.stringify(pages, null, 1) + '\n');
await fs.writeFile('site-ghost/export/posts.json', JSON.stringify(posts, null, 1) + '\n');
