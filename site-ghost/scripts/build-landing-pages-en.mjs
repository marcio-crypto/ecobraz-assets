...
const pages = JSON.parse(await fs.readFile(path.join(root, 'landing', 'landing-pages-en.json'), 'utf8'));
...
const referenciaSection = (p, slug) => !p.referencia ? '' : `<section class="hx-block">
  <div class="container">
    <div class="hx-head-split">
      <div><span class="hx-label">Technical reference</span><h2>Published basis, with a persistent identifier</h2></div>
      <p>This subject is developed in an original technical report by Ecobraz Emigre, published with a DOI on the open Zenodo repository.</p>
    </div>
    <p style="margin:0">VILLANOVA, Marcio. <em>${esc(p.referencia.titulo)}</em>. Zenodo, 2026. <a class="hx-src" href="https://doi.org/${p.referencia.doi}" rel="noopener" data-track="${slug}_referencia_doi">doi.org/${p.referencia.doi}</a> &nbsp;·&nbsp; <a class="hx-src" href="{{@site.url}}/technical-publications/" data-track="${slug}_referencia_publicacoes">All publications →</a></p>
  </div>
</section>

`;
const referenciaHtml = (p) => !p.referencia ? '' : `<hr><h2>Technical reference</h2><p>This subject is developed in an original technical report by Ecobraz Emigre, published with a persistent identifier (DOI) on the open Zenodo repository:</p><ul><li>VILLANOVA, Marcio. <em>${esc(p.referencia.titulo)}</em>. Zenodo, 2026. <a href="https://doi.org/${p.referencia.doi}" rel="noopener">doi.org/${p.referencia.doi}</a>.</li></ul><p><a href="/technical-publications/">See all of Ecobraz\u2019s technical publications →</a></p>`;

function renderTemplate(p) {
  const slug = p.slug;
  const formAction = '{{@site.url}}/request-a-collection/';
...
<section class="hx-hero">
  <div class="container hx-crumbs"><a href="{{@site.url}}/">Home</a> › <a href="{{@site.url}}${p.hub.href}">${esc(p.hub.title)}</a> › ${esc(p.crumb)}</div>
  <div class="container hx-hero-grid">
    <div>
      ...
      <div class="hx-hero-ctas">
        <a class="button" href="{{@site.url}}/request-a-collection/?perfil=empresa&amp;origem=${slug}${p.form.material ? `&amp;material=${encodeURIComponent(p.form.material)}` : ''}" data-track="${slug}_hero_cta">${esc(p.hero.cta)}</a>
        <a class="button hx-btn-outline-dark" href="#como-funciona" data-track="${slug}_hero_como">How it works</a>
      </div>
    ...
    <form class="hx-quote" id="avaliacao" method="get" action="${formAction}">
      <span class="hx-label">Technical assessment</span>
      ...
      <div class="hx-field">
        <label for="lp-local">Postcode or collection city</label>
        <input id="lp-local" name="local" placeholder="e.g. 02175-010 or São Paulo">
      </div>
      <button class="button" type="submit" data-track="${slug}_hero_form">${esc(p.form.button || 'Request a technical assessment')} →</button>
      <div class="hx-or-wa">or <a href="{{@custom.whatsapp_url}}" rel="noopener" data-track="${slug}_hero_whatsapp">${wa} talk to the team on WhatsApp</a></div>
      <p class="hx-micro">${esc(p.form.micro || 'Collection is confirmed after assessment of material, volume, location and documentation.')}</p>
    </form>
  </div>
</section>
...
${p.recognize ? `<section class="hx-block">
  <div class="container hx-reco">
    <div>
      <span class="hx-label">Does this sound like you?</span>
      <h2>${esc(p.recognize.title)}</h2>
      <p class="hx-reco-cta">Is even one item on this list true for you? Then this page was written for you — <a href="#avaliacao">describe your situation in one minute</a> and get a technical reply, not a sales call.</p>
    </div>
...
` : ''}<section class="hx-block${p.recognize ? ' alt' : ''}">
  <div class="container">
    <div class="hx-head-split">
      <div><span class="hx-label">What is at stake</span><h2>${esc(p.painsTitle)}</h2></div>
      <p>${esc(p.painsSub)}</p>
    </div>
...
${p.contrast ? `<section class="hx-block">
  <div class="container">
    <div class="hx-head-split">
      <div><span class="hx-label">The turning point</span><h2>${esc(p.contrast.title)}</h2></div>
...
` : ''}${p.gains ? `<section class="hx-block alt">
  <div class="container">
    <div class="hx-head-split">
      <div><span class="hx-label">What you gain</span><h2>${esc(p.gains.title)}</h2></div>
      <p>${esc(p.gains.sub)}</p>
    </div>
...
` : ''}<section class="hx-block">
  <div class="container">
    <div class="hx-head-split">
      <div><span class="hx-label">Clear scope</span><h2>${esc(p.scope.title || 'What is included — and what is not')}</h2></div>
      <p>${esc(p.scope.sub || 'Scope declared before scheduling reduces rework and surprises. Each batch is assessed technically.')}</p>
    </div>
    <div class="hx-scope">
      <div class="hx-yes"><h3>${esc(p.scope.inTitle || 'In scope')}</h3><ul>${scopeIn}</ul><p>${esc(p.scope.conditions)}</p></div>
      <div class="hx-not"><h3>${esc(p.scope.outTitle || 'Out of scope')}</h3><ul>${scopeOut}</ul><p>${esc(p.scope.outNote || 'Materials out of scope require specific chains and are not part of this service.')}</p></div>
    </div>
  </div>
</section>
...
<section class="hx-block alt" id="como-funciona">
  <div class="container">
    <div class="hx-head-split">
      <div><span class="hx-label">Process</span><h2>${esc(p.stepsTitle || 'How it works, from contact to final destination')}</h2></div>
      <p>${esc(p.stepsSub || 'No collection is confirmed without a technical assessment. That is what keeps the operation predictable and documentable.')}</p>
    </div>
...
<section class="hx-block">
  <div class="container">
    <div class="hx-head-split">
      <div><span class="hx-label">Evidence of the operation</span><h2>${esc(p.evidenceTitle)}</h2></div>
      <p>The documentation reflects what was actually carried out in each batch — no generic promises.</p>
    </div>
    <div class="hx-docs">
        ${docs}
    </div>
...
${p.why ? `<section class="hx-block alt">
  <div class="container">
    <div class="hx-head-split">
      <div><span class="hx-label">Why Ecobraz</span><h2>${esc(p.why.title)}</h2></div>
      <p>${esc(p.why.sub)}</p>
    </div>
    <div class="hx-sol-grid g4">
        ${why}
...
` : ''}<section class="hx-block" style="padding-top:0">
  <div class="container">
    <div class="hx-authority">
      <div>
        <span class="hx-label on-dark">Verifiable authority</span>
        <strong>Associação Auxílio à Reciclagem de Eletrônicos e Inclusão Digital — company ID (CNPJ) 14.197.457/0001-42</strong>
        <p>${esc(p.authority)}</p>
      </div>
      <a href="{{@site.url}}/public-evidence/" data-track="${slug}_evidencias">See public evidence →</a>
    </div>
  </div>
</section>
...
<section class="hx-block alt">
  <div class="container">
    <div class="hx-head-split">
      <div><span class="hx-label">Frequently asked questions</span><h2>${esc(p.faqTitle)}</h2></div>
    </div>
    <div class="hx-faq">
        ${faq}
    </div>
...
<section class="hx-block">
  <div class="container">
    <div class="hx-head-split">
      <div><span class="hx-label">Related solutions</span><h2>Could also be part of your operation</h2></div>
    </div>
    <div class="hx-sol-grid">
        ${related}
    </div>
  ...
  <div class="container">
    <div class="hx-final">
      ...
      <div class="hx-actions">
        <a class="button" href="{{@site.url}}/request-a-collection/?perfil=empresa&amp;origem=${slug}${p.form.material ? `&amp;material=${encodeURIComponent(p.form.material)}` : ''}" data-track="${slug}_final_cta">${esc(p.hero.cta)}</a>
...
function renderHubTemplate(p) {
  ...
  <div class="container">
    ...
    <div class="hx-sol-grid">
        ${g.items.map((i) => `<a class="hx-sol" href="{{@site.url}}${i.href}"><span class="hx-icon">${icons[i.icon] || icons.box}</span><h3>${esc(i.title)}</h3><p>${esc(i.text)}</p><span class="hx-go">View page →</span></a>`).join('\n        ')}
    </div>
  </div>
</section>`).join('\n\n');
...
<section class="hx-hero">
  <div class="container hx-crumbs"><a href="{{@site.url}}/">Home</a> › ${esc(p.crumb)}</div>
  <div class="container hx-hero-grid" style="grid-template-columns:1fr">
    <div>
      ...
      <p class="hx-sub">${esc(p.hero.sub)}</p>
      <div class="hx-hero-ctas">
        <a class="button" href="{{@site.url}}/request-a-collection/?perfil=empresa&amp;origem=${slug}" data-track="${slug}_hero_cta">${esc(p.hero.cta)}</a>
        <a class="button hx-btn-outline-dark" href="{{@custom.whatsapp_url}}" rel="noopener" data-track="${slug}_hero_whatsapp">Talk on WhatsApp</a>
      </div>
    </div>
  </div>
</section>
...
${referenciaSection(p, slug)}<section class="hx-block alt">
  <div class="container">
    <div class="hx-final">
      ...
      <div class="hx-actions">
        <a class="button" href="{{@site.url}}/request-a-collection/?perfil=empresa&amp;origem=${slug}" data-track="${slug}_final_cta">${esc(p.hero.cta)}</a>
        <a class="button hx-btn-wa" href="{{@custom.whatsapp_url}}" rel="noopener" data-track="${slug}_final_whatsapp">${wa} WhatsApp</a>
      </div>
    </div>
  </div>
...
function renderHubSyncEntry(p) {
  ...
  const html = [
    ...
    `<p><a href="mailto:contato@ecobraz.org.br">${esc(p.hero.cta)}</a>.</p>`
...
function renderSyncEntry(p) {
  ...
  const html = [
    ...
    `<p>${esc(p.painsSub)}</p>`,
    `<h2>Scope</h2><p>${esc(p.scope.conditions)}</p>`,
    `<p>Hub solution <a href="${p.hub.href}">${esc(p.hub.title)}</a>. Related: ${p.related.map((r) => `<a href="${r.href}">${esc(r.title)}</a>`).join(', ')}. Evidence: <a href="/documentation-traceability/">documentation and traceability</a> and <a href="/public-evidence/">public evidence</a>.</p>`,
    `<p><a href="mailto:contato@ecobraz.org.br">Request a technical assessment</a>.</p>`
  ].join('') + referenciaHtml(p);
  return {
    title: p.title,
    slug: p.slug,
...
for (const p of pages) {
  ...
  console.log('Generated (EN) page-%s.hbs%s', p.slug, isHub ? ' (hub)' : '');
}
await fs.writeFile(path.join(root, 'content', 'en-commercial-pages.json'), JSON.stringify(syncEntries, null, 2) + '\n');
console.log('Generated content/en-commercial-pages.json with %d pages.', syncEntries.length);
