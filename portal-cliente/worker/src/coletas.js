...
export function paginaColetasLista(user, coletas, q, cliCtx, negocios) {
  ...
  const fmtTamC = (n) => { n = Number(n || 0); return n ? (n >= 1048576 ? (n / 1048576).toFixed(1) + ' MB' : Math.max(1, Math.round(n / 1024)) + ' KB') : ''; };
  const histBlock = negs.length ? `<div style="margin-top:20px"><div style="font-size:12px;font-weight:800;color:#7c8a87;text-transform:uppercase;letter-spacing:.04em;margin-bottom:8px">Histórico anterior — migrado do Ploomes · ${negs.length}</div>${negs.map((n) => { const docs = n.arquivos || []; const dl = docs.length ? docs.map((a) => `<a href="/cadastro/arquivo?key=${encodeURIComponent(a.r2_key)}&nome=${encodeURIComponent(a.nome_arquivo || '')}" target="_blank" rel="noopener" style="display:flex;justify-content:space-between;align-items:center;gap:8px;text-decoration:none;border:1px solid #EEF1F0;border-radius:8px;padding:8px 10px;margin:6px 0 0;background:#fff"><span style="min-width:0;font-size:12px;color:#10262B;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">📄 ${esc(a.nome_arquivo || a.r2_key)}</span><span style="flex:none;font-size:10.5px;color:#0B5B66;font-weight:700">${fmtTamC(a.tamanho) ? fmtTamC(a.tamanho) + ' · ' : ''}abrir ↗</span></a>`).join('') : '<div style="font-size:11.5px;color:#8fa39f;padding:6px 0 2px">Sem documentos vinculados a esta OS.</div>'; return `<details style="border:1px solid #E4EBE9;border-radius:12px;padding:2px 15px;margin-bottom:8px;background:#FBFDFC"><summary style="cursor:pointer;list-style:none;display:flex;justify-content:space-between;align-items:center;gap:10px;padding:11px 0"><span style="min-width:0"><span style="font-size:13px;font-weight:700;color:#10262B">${esc(n.titulo)}</span><span style="display:block;font-size:11.5px;color:#8fa39f">${esc(dataBR(n.data))}${docs.length ? ` · ${docs.length} documento(s) — toque para abrir` : ''}</span></span><span style="flex:none;font-size:10px;font-weight:800;color:${n.cor || '#8A6A16'}">${esc(String(n.status || '').toUpperCase())}</span></summary><div style="padding:0 0 10px">${dl}</div></details>`; }).join('')}</div>` : '';
